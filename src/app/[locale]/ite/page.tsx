...
import { ArrowRight, CheckCircle2, Home, Euro, Shield } from "lucide-react";
import { getTranslations } from "next-intl/server";

export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "ITE" });
  return {
      ...
      <section className="py-12 bg-white border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="flex items-center p-6 bg-gray-50 rounded-xl border border-gray-100">
              <Home className="w-10 h-10 text-brand-green mr-4 flex-shrink-0" />
